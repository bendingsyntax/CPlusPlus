...
int main()
{
   ...
   for (int snum = numStudents; snum >= 1; snum--)    //Outer loop
   {  double total = 0.0;   // Initialize accumulator 
									  
      for (int test = numTests; test >= 1; test--)    //Inner loop
      {  int score;
      ...
      cout << " is " << average << "\n\n";
	  studentsAverage[snum] = average;
   }                                                  //End outer loop
   double total = 0.0;
   int snum = 1;
   for (; snum <= numStudents; snum++)
	   total += studentsAverage[snum];
   cout << "The average class average is "
	   << total / --snum << "\n\n";
   return 0;
}
